create or replace procedure  month_avg_incexp  as cursor c1 is
select avg(amount)amt,to_char(feespaiddate,'MON') paydt from feespaid 
group by to_char(feespaiddate,'MON');
cursor c2 is
select avg(amount)amt,to_char(expdate,'MON') expdt from expenditure
	...
	for z1 in c1 loop
		...
		select nvl(month,null) into indt from month_avgincexp where 				month=z1.paydt;
     		if (indt <> ' ') then
     			...
     			insert into month_avgincexp (month, income) 							values(z1.paydt, z1.amt);
     		end if;
	end loop;
     	for z2 in c2 loop
     		y := z2.expdt;
  		select nvl(month,null) into expdt from month_avgincexp where 				month=z2.expdt;
     		if (expdt <> ' ') then
     			...
     			insert into month_avgincexp (month, expenditure) 						values(z2.expdt, z2.amt);
     		end if;
	end loop;
end month_avg_incexp;
/
